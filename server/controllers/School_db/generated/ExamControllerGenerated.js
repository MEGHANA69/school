...
const generatedControllers = {
  ...
  init: router => {
    ...
    router.get(baseUrl + "/findBy_courses/:key", authorize([]), ExamController.findBy_courses);
  ...
  */
  findBy_courses: async (req, res) => {
    try {
      const result = await ExamModel.findBy_courses(req.params.key);
      res.json(result);
    } catch (err) {
      const safeErr = ErrorManager.getSafeError(err);
      res.status(safeErr.status).json(safeErr);
